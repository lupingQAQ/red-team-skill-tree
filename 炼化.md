...
**主机选购**: 非大陆地区(shockhosting/psychz.net/亚马逊云/CF/jtti.cc)，最好支持USDT，不同主机商分散，避免香港阿里云
...
**创建账号**: GitHub / SecurityTrails(全球最大情报商) / Censys(每天更新+rustscan) / FOFA-Hunter(不允许实战虚拟机内登录)
...
**商业间谍**: 如何买通内鬼 / 商业咨询公司套路 / 如何查处内鬼 / 料子上下游分析溯源
...
| FOFA / Hunter | ico/title/body搜索；不允许实战虚拟机内登录 |
...
| **客服站** | XSS→钓鱼/electron RCE / 买的源码自己搭→markdown标签 / 找客服系统供应商套路测试站源码 / 找后台/扫目录 / 通过客服站资产归因包网资产(94chat) / 套路客服获取站点更多信息 |
| **页面关联** | JS / JS Console / 静态资源加载 / WSS (不靠谱) |
| **包网运维** | Jenkins / 各类国产OA管理软件 / MQ / 测试站弱口令→GETShell→横向/挂马钓鱼/拿源码/报错调试 / 收集后台接口(../../../) / 源码二道贩子(简单聊/扫备份/黑盒/根据后台页面加载资源找更多页面特征→money.php/后台调试报错→特殊数据库表名文件名) |
...
| 演示站 | 打源码 |
| 脱裤 | adminer / 拖到服务器分片传输(web目录/GitHub LFS/Action) |
...
**漏洞利用方法论**:
```
组件→源码(开源/备份/套路客服/网盘泄露/多版本测试/商业版破解)→版本(更新时间/修复日志→绕过/影响范围/商业版差异)
...
## 18. 【2025-2026新增】Cloud Post-Exploitation
...
## 23. 【2025-2026新增】Living off the Land (LotL) 攻击链
